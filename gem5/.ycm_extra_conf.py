...
def FindCorrespondingSourceFile( filename ):
    res = filename.replace('src/', 'build/RISCV/')

    suffix = IsHeaderFile(filename)
    if suffix:
        for i in SOURCE_EXTENSIONS:
            temp = res.replace(suffix, i)
            if p.exists(temp):
                res = temp

    logging.log(logging.WARN, 'Flag is from: %s, %d', res, int(p.exists(res)))
    return res

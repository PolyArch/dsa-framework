...
void input_to_data(int fd, void *vdata) {
  ...
  parse_int64_t_array(s, data->cols, NNZ);

  s = find_section_start(p,3);
  parse_int64_t_array(s, data->rowDelimiters, N+1);
...
void data_to_input(int fd, void *vdata) {
  ...
  write_section_header(fd);
  write_int64_t_array(fd, data->cols, NNZ);

  write_section_header(fd);
  write_int64_t_array(fd, data->rowDelimiters, N+1);

...
void svd(complex<float> *a, complex<float> *u, float *s, complex<float> *v) {
  for (int i = 0; i < _N_ - 1; ++i) {
    ...
    if (i != _N_ - 2) {
      ...
      SS_2D_CONST(P_mvc_reset, 2, len - 1, 1, 1, len);
      SS_DMA_WRITE(P_mvc_O, 8, 8, len, temp);
      //for (int j = 0; j < len; ++j) {
      ...
      SS_REPEAT_PORT(len);
      SS_DMA_READ(temp, 8, 8, len, P_vv_B);
      //for (int j = 0; j < len; ++j) {
      ...
      if (!i) {
        ...
        SS_2D_CONST(P_vvc_C, 1065353216, 1, 0, _N_ - 1, _N_ - 2);
        SS_CONST(P_vvc_C, 0, 1);

        SS_REPEAT_PORT(_N_ - 1);
        SS_DMA_READ(hv, 8, 8, _N_ - 1, P_vvc_A);
        //for (int j = 1; j < _N_; ++j) {
        ...
        SS_REPEAT_PORT(_N_ - 1);
        SS_DMA_READ(hv, 8, 8, _N_ - i - 1, P_vm_A);
        ...
        SS_REPEAT_PORT(_N_ - 1);
        SS_DMA_READ(hv, 8, 8, len, P_vvc_A);
        //for (int k = 0; k < len; ++k) {
  ...
  SS_DMA_WRITE(P_finalize_O, 0, 8 * _N_ * _N_, 1, u);
  for (int i = 0; i < _N_; ++i) {
